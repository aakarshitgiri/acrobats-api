...
exports.exportPdfToImggs = async (req, res, next) => {
    ...
    try {
        ...
        const executionContext = await PDFServicesSdk.ExecutionContext.create(credentials),
            exportPDFToImages = await PDFServicesSdk.ExportPDFToImages,
            exportPDFToImagesOperation = await exportPDFToImages.Operation.createNew(exportPDFToImages.SupportedTargetFormats.JPEG);

        // Set the output type to create zip of images.
        await exportPDFToImagesOperation.setOutputType(exportPDFToImages.OutputType.ZIP_OF_PAGE_IMAGES);

        // Set operation input from a source file
        const input = await PDFServicesSdk.FileRef.createFromLocalFile(link);
        await exportPDFToImagesOperation.setInput(input);
        ...
        async function createOutputFilePath() {
            ...
            return ("../output/tensech-images-from-pdf/export" + dateString + ".zip");
        }
        return res.json({ message: "PDF Export pdf to jpg in zip Successfully", success: true });
    } catch (err) {
        console.log('Exception encountered while executing operation', err);
    }
