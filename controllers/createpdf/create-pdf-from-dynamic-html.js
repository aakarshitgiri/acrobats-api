...
exports.webToPdf = async (req, res, next) => {
    ...
    try {
        ...
        const input = PDFServicesSdk.FileRef.createFromLocalFile(link);
        ...
        async function createOutputFilePath() {
            let date = new Date();
            let dateString = await date.getFullYear() + "-" + ("0" + (date.getMonth() + 1)).slice(-2) + "-" +
                ("0" + date.getDate()).slice(-2) + "T" + ("0" + date.getHours()).slice(-2) + "-" +
                ("0" + date.getMinutes()).slice(-2) + "-" + ("0" + date.getSeconds()).slice(-2);
            return ("../output/dynamic-html/tensech" + dateString + ".pdf");
        }
        return res.json({ message: "PDF Created Successfully", success: true });
