...
exports.extractTextFromPdf = async (req, res, next) => {
    ...
    try {
        ...
        const extractPDFOperation = await PDFServicesSdk.ExtractPDF.Operation.createNew(),
            input = await PDFServicesSdk.FileRef.createFromLocalFile(
                link,
        ...
        async function createOutputFilePath() {
            ...
            return ("../output/tensect-extract-text/extract" + dateString + ".zip");
        }
        return res.json({ message: "PDF extract text from pdf Successfully", success: true });
    } catch (err) {
        console.log('Exception encountered while executing operation', err);
    }
}